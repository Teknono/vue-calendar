<template>
  <div id="app" class="container">
    ...
    <div class="headers row" :class="{ 'dim': noCitiesHaveLocation }">
      <div class="element add-remove">
        <button class="circle" @click="addCity">+</button>
      </div>
      <div class="element label" :class="header" v-for="header in weatherAttributes" :key="header">
    ...
    <transition-group name="wx-list">
      <Weather :city="city" v-for="city in cities" :key="city.id"></Weather>
    </transition-group>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    sortBy(attribute) {
      this.$store.dispatch('sortBy', attribute)
    },
    currentlySorting(header) {
      return this.sortingBy === header
    },
    direction() {
...
body {
  ...
  line-height: 1.34;
  color: #333;
  font-family: "Whitney Narrow SSm A", "Whitney Narrow SSm B", sans-serif;
}
...
.wx-data-enter-active {
  animation: fadeInUp .5s;
}

.wx-data-leave-active {
...
@media only screen and (max-width: 480px) {
  ...
  .weather {
    ...
    &.row:nth-child(odd) {
      background-color: hsla(205, 35, 45, 0.1);
    }
  }
  input,
  p,
